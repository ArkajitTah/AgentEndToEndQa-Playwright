# End-to-End QA Workflow with Natural Language (Custom Agents)

## Workflow Overview

This prompt guides you through a complete 7-step QA workflow using MCP servers and your own custom AI agents (qa-test-planner, qa-test-generator, qa-test-healer) to go from user story to committed automated test scripts.
...
```
Based on the user story SCRUM-101 that we just reviewed, use the @qa-test-planner agent to:
...
```
Now I need to create automated test scripts using the @qa-test-generator agent.

Please review:
1. Test plan from: specs/saucedemo-checkout-test-plan.md (for test scenarios and steps)
2. Exploratory testing results from Step 3 (for actual selectors and UI insights)
...
```
Now I need to execute the generated automation scripts and heal any failures using the @qa-test-healer agent.

1. Run all automation scripts in: tests/saucedemo-checkout/
2. Identify any failing tests
3. For each failing test, use the @qa-test-healer agent to:
...
- Failing tests identified and healed using @qa-test-healer agent
...
**Git Repository URL:** https://github.com/ArkajitTah/AgentEndToEndQa-Playwright.git
...
```
I want to demonstrate a complete end-to-end QA workflow using natural language, MCP servers, and my own custom agents.
...
STEP 2 - CREATE TEST PLAN:
Use the @qa-test-planner agent to create a comprehensive test plan based on the user story. The agent should explore the application URL from the user story and cover all acceptance criteria. Save it as: specs/saucedemo-checkout-test-plan.md
...
Review both the test plan (specs/saucedemo-checkout-test-plan.md) and exploratory testing results from Step 3. Use the @qa-test-generator agent to create JavaScript automation scripts leveraging the element selectors and insights discovered during manual testing. Save scripts in tests/saucedemo-checkout/.

STEP 5 - EXECUTE AND HEAL TESTS:
Run all automation scripts from tests/saucedemo-checkout/. Use the @qa-test-healer agent to identify and auto-heal any failing tests. Re-run tests until all are stable and passing. Document healing activities.
